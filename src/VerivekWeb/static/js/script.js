...
function openDatasetImportModal() {
    document.getElementById('dataset-import-modal').classList.remove('hidden');
    resetDatasetUploadUI();
}

function closeDatasetImportModal() {
    document.getElementById('dataset-import-modal').classList.add('hidden');
}
...
function handleDatasetFileSelect(event) {
    ...
    if (files.length > 0) {
        ...
        document.getElementById('dataset-file-type').textContent = isFolder ?
...
function resetDatasetUploadUI() {
    document.getElementById('dataset-import-form').reset();
    document.getElementById('dataset-file-type').textContent = '';
    document.getElementById('upload-progress-container').classList.add('hidden');
    document.getElementById('upload-progress-bar').style.width = '0%';
    document.getElementById('upload-percentage').textContent = '0%';
}
...
async function handleDatasetImport(event) {
    ...
    formData.append('message', document.getElementById('import-message').value);
    ...
    const progressInterval = setInterval(() => {
        if (progress < 90) {
            ...
            percentageText.textContent = `${Math.round(Math.min(progress, 90))}%`;
        }
    }, 300);
    ...
    try {
        ...
        if (response.ok) {
            ...
            setTimeout(() => {
                ...
                if (typeof fetchDatasets === 'function') fetchDatasets();
                resetDatasetUploadUI();
                submitBtn.disabled = false;
            }, 500);
        } else {
...
async function fetchDatasets() {
    ...
    const loading = document.getElementById('dataset-list-loading');
